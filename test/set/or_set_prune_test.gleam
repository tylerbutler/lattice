...
pub fn prune_removes_tombstones_test() {
  let s1 = 
    ...
    |> or_set.add("item1") 
    |> or_set.add("item2")
    |> or_set.remove("item1") // Creates tombstone for item1 (tag: A:1)
...
pub fn prune_prevents_zombies_test() {
  ...
  let s_pruned = 
    or_set.new("A")
    |> or_set.add("item1") // A:1
    |> or_set.remove("item1")
    |> or_set.prune(version_vector.new() |> version_vector.increment("A")) // Prune A:1
...
pub fn json_serialization_v2_test() {
  ...
  let s_zombie = or_set.new("A") |> or_set.add("item1") // A:1
