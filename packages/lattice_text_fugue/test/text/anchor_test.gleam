...
pub fn concurrent_insert_unicode_order_and_anchor_test() {
  let bmp = text.new(rid("\u{e000}")) |> text.insert(0, "b")
  let supplementary = text.new(rid("\u{10000}")) |> text.insert(0, "s")
  let anchor = text.anchor_at(bmp, 0, sequence.Before)

  [text.merge(bmp, supplementary), text.merge(supplementary, bmp)]
  |> list.each(fn(merged) {
    text.value(merged) |> expect.to_equal("bs")
    text.resolve_anchor(merged, anchor) |> expect.to_equal(0)
  })
}
